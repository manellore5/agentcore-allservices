// Preloaded telemetry for Deno agents on AgentCore Runtime: `deno run --preload observability.ts main.ts`.
// Agent code imports nothing from here. Traces -> X-Ray OTLP, logs -> CloudWatch Logs OTLP, both SigV4-signed.
...
const ENABLED = env("AGENT_OBSERVABILITY_ENABLED") === "true" && env("DISABLE_ADOT_OBSERVABILITY") !== "true";
...
async function signedPost(service: string, path: string, body: Uint8Array, extra: Record<string, string> = {}) {
  ...
  const res = await fetch(`https://${host}${path}`, {
    method: "POST", headers: signed.headers as Record<string, string>, body: body as BodyInit,
  });
  const text = await res.text();
  if (!res.ok) console.log(JSON.stringify({ observability: "export_failed", service, status: res.status, body: text.slice(0, 300) }));
  return res.ok;
}
...
class SigV4SpanExporter implements SpanExporter {
  export(spans: ReadableSpan[], done: (r: { code: number }) => void) {
    ...
    signedPost("xray", "/v1/traces", body).then((ok) => done({ code: ok ? 0 : 1 }), () => done({ code: 1 }));
  }
...
class SigV4LogExporter implements LogRecordExporter {
  ...
  export(logs: ReadableLogRecord[], done: (r: { code: number }) => void) {
    ...
    for (const k of ["x-aws-log-group", "x-aws-log-stream"]) if (this.#headers[k]) extra[k] = this.#headers[k];
    signedPost("logs", "/v1/logs", body, extra).then((ok) => done({ code: ok ? 0 : 1 }), () => done({ code: 1 }));
  }
...
class ConversationLogProcessor implements SpanProcessor {
  ...
  onEnd(span: ReadableSpan) {
    ...
    for (const ev of span.events) {
      ...
      else if (ev.name === "gen_ai.choice") output.push({ role: "assistant", content: String(a.message ?? a.content ?? "") });
    }
    ...
    this.logs.getLogger(scope).emit({
      ...
      attributes: { "event.name": scope, "session.id": String(span.attributes["session.id"] ?? "") },
    });
  }
  forceFlush() { return Promise.resolve(); }
  shutdown() { return Promise.resolve(); }
    ...
    if (i >= 0) {
      ...
      args[i] = (req: http.IncomingMessage, res: http.ServerResponse) => {
        ...
        if (typeof sid === "string" && sid) {
          const bag = (propagation.getBaggage(ctx) ?? propagation.createBaggage()).setEntry("session.id", { value: sid });
...
if (ENABLED) {
  ...
  const logs = new LoggerProvider({
    resource,
    processors: [new BatchLogRecordProcessor(new SigV4LogExporter(), { scheduledDelayMillis: 1000 })],
  });
